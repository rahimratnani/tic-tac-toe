...
const Gameplay = (function (doc) {
    ...
    const handleNormalMode = e => {
        if (e.target.textContent === '' && !isGameOver) {
            Gameboard.setLocations(Number(e.target.getAttribute('data-tile')), getCurrentMarker());
            render();
    ...
    const emptyPlaces = gameArr => { // returns array of empty places
    ...
    const minimax = (newBoard, player) => {
        ...
        for (let i = 0; i < availSpots.length; i++) {
            ...
            if ((player === player2.getMarker() && move.score === 10) || (player === player1.getMarker() && move.score === -10)) {
                return move;
            } else {
                moves.push(move);
            }
